...
#Split edges into Eukaryote-Bacteria only
eukaryote_prokaryote_edges <- edges %>% filter(paste0(substring(from_ASV,1,3),substring(to_ASV,1,3)) == "EukBac" |
                                    paste0(substring(from_ASV,1,3),substring(to_ASV,1,3)) == "EukArc")

#Only get interactions between different clusters
eukaryote_prokaryote_edges_from_to_different <- eukaryote_prokaryote_edges[eukaryote_prokaryote_edges$from_clu != eukaryote_prokaryote_edges$to_clu,]



## (Not used currently)
# Number of interactions of each ASV
#euk_bac_out_interactions_per_asv <- data.frame(table(eukaryote_prokaryote_edges_from_to_different$from_ASV))
#euk_bac_in_interactions_per_asv <- data.frame(table(eukaryote_prokaryote_edges_from_to_different$to_ASV))



## Number of interactions from/to different clusters
# Make adjacency matrix
adjacencyData <- with(eukaryote_prokaryote_edges_from_to_different, table(from_clu, to_clu)) %>% as.data.frame()
...
dodge_number_no_values <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Interactions", title = "Eukaryote-Prokaryote Interactions")+
...
stacked_percent <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Distribution of in and outgoing interactions",
       title= "Eukaryote-Prokaryote Interaction Ratio")+
  guides(fill=guide_legend(title="Interaction\n.... cluster"))
