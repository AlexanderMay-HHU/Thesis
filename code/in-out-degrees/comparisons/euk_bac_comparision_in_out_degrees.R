...
stacked_number <- ggplot(in_out_deg, aes(fill=interaction_type, y=count, x=cluster))+
  ...
  labs(title="Interactions from Eukaryotes to Bacteria of each Cluster", y="Count",x="Cluster")+
  geom_text(aes(label=count), position = position_stack(vjust= 0.5), check_overlap = TRUE)

stacked_number

#Save to plot_path
ggsave(filename="Comparision_euk_bac_in_out_degrees_count.png", plot=stacked_number, path=paste(plot_path,"/InOutDegrees/Comparisons/Count/",sep=""))
...
stacked_percent <- ggplot(in_out_deg, aes(fill=interaction_type, y=count, x=cluster))+
  ...
  scale_fill_manual(values=c("#009900","#990000"))+
  labs(title="Interactions from Eukaryotes to Bacteria of each Cluster",y="Distribution of in and outgoing interactions",
       x="Cluster")

stacked_percent

#Save to plot_path
ggsave(filename="Comparision_euk_bac_in_out_degrees_percent.png", plot=stacked_percent, path=paste(plot_path,"/InOutDegrees/Comparisons/Percent/",sep=""))
