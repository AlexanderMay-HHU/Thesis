...
bac_edges <- edges %>% filter(paste0(substring(from_ASV,1,3),substring(to_ASV,1,3)) == "BacBac")
...
adjacencyData <- with(bac_edges_from_to_different, table(from_clu, to_clu)) %>% as.data.frame()
...
#Save to plot_path
ggsave(filename="Bac_Bac.png", plot=stacked_number, path=paste(plot_path,"/Count/",sep=""))
...
#Save to plot_path
ggsave(filename="Bac_Bac.png", plot=stacked_percent, path=paste(plot_path,"/Percent/",sep=""))