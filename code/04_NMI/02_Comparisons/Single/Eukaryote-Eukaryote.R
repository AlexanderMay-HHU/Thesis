...
#Split edges into Eukaryote only
...
#out_interactions_per_asv <- data.frame(table(eukaryote_edges_from_to_different$from_ASV))
#in_interactions_per_asv <- data.frame(table(eukaryote_edges_from_to_different$to_ASV))



## Number of interactions from/to different clusters
# Make adjacency matrix
adjacencyData <- with(eukaryote_edges_from_to_different, table(from_clu, to_clu)) %>% as.data.frame()
...
dodge_number_no_values <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Interactions", title = "Eukaryote-Eukaryote Interactions")+
...
stacked_percent <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Distribution of in and outgoing interactions",
       title= "Eukaryote-Eukaryote Interaction Ratio")+
  guides(fill=guide_legend(title="Interaction\n.... cluster"))+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
