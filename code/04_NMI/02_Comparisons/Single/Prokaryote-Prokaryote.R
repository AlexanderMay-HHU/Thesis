...
#out_interactions_per_asv <- data.frame(table(prokaryote_edges_from_to_different$from_ASV))
#in_interactions_per_asv <- data.frame(table(prokaryote_edges_from_to_different$to_ASV))



## Number of interactions from/to different clusters
# Make adjacency matrix
adjacencyData <- with(prokaryote_edges_from_to_different, table(from_clu, to_clu)) %>% as.data.frame()
...
dodge_number_no_values <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Interactions", title = "Prokaryote-Prokaryote Interactions")+
...
#Save to plot_path
ggsave(filename="Prokaryote-Prokaryote_Count.png", plot=dodge_number_no_values,
       path=paste0(plot_path,"04_NMI/02_Comparisons/Single/"))
ggsave(filename="Prokaryote-Prokaryote_Count_values.png", plot=dodge_number,
       path=paste0(plot_path,"00_Appendix/04_NMI/02_Comparisons/Single/"))
...
stacked_percent <- ggplot(in_out_deg, aes(fill=interaction_type,
  ...
  labs(x="Cluster", y="Distribution of in and outgoing interactions",
       title= "Prokaryote-Prokaryote Interaction Ratio")+
  guides(fill=guide_legend(title="Interaction\n.... cluster"))+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
...
#Save to plot_path
ggsave(filename="Prokaryote-Prokaryote_Percent.png", plot=stacked_percent,
       path=paste0(plot_path,"04_NMI/02_Comparisons/Single/"))