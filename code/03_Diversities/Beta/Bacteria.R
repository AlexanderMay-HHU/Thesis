...
abundance_of_phylum_in_cluster <- data.frame(matrix(ncol=length(unique(nodes_bacteria$Phylum)),nrow=length(unique(nodes_bacteria$LouvainLabelD))))
colnames(abundance_of_phylum_in_cluster) <- unique(nodes_bacteria$Phylum)
rownames(abundance_of_phylum_in_cluster) <- sort(unique(nodes_bacteria$LouvainLabelD))
...
for (cluster in sort(unique(nodes_bacteria$LouvainLabelD))){
  for (phylum in unique(nodes_bacteria$Phylum)){
    ...
    if(!is.na(phylum)){
      abundance_of_phylum_in_cluster[ifelse(cluster==0,1,cluster),phylum] <- sum(nodes_bacteria[nodes_bacteria$LouvainLabelD == cluster &
...
heatmap_full <- ggplot(bray_curtis_dist, aes(x = Var2,
                  ...
                  labs(x = "Cluster", y = "Cluster", title = "Bacteria") + # Labels
...
ggsave(filename="Bray_Curtis_Full.png", plot=heatmap_full, path=paste0(plot_path,"03_Diversities/Beta/Prokaryotes/Bacteria"))
ggsave(filename="Bray_Curtis_Lower.png", plot=heatmap_lower, path=paste0(plot_path,"00_Appendix/03_Diversities/Beta/Prokaryotes/Bacteria"))