...
for (cluster in sort(unique(nodes_bac$LouvainLabelD))){
  ...
  #Chao1 - Richness
  chao1 <- c(chao1,chao1(nodes_bac[which(nodes_bac$LouvainLabelD == cluster),1],taxa.row=TRUE))
  #Shannon Entropy
  shannon <- c(shannon,diversity(nodes_bac[which(nodes_bac$LouvainLabelD == cluster),1],index="shannon"))
  #Simpson-Index
  simpson <- c(simpson,diversity(nodes_bac[which(nodes_bac$LouvainLabelD == cluster),1],index="simpson"))
...
ASV_count <- nodes_bac %>% group_by(LouvainLabelD) %>% summarise(ASVs = n_distinct(Sequence)) %>% select(ASVs)
df_diversity <- nodes_bac %>% group_by(LouvainLabelD) %>% summarise(Abundance = sum(Abundance4y)) %>%
